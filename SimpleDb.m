...
- (void) queryWithAttributes:(NSString*)domainName expression:(NSString*)expression {
	...
	if ([parser parse] == NO)
	{
		...
		NSLog(@"SimpleDB: Parsing output failed. Error Code: %d - Domain: %@ - UserInfo: %@", [error code], [error domain], [error userInfo] );
	}
	else
	{
		NSLog(@"SimpleDB: Parsing output successful");
	}
...
- (NSXMLParser*) invoke:(NSString*)methodName invokeParameters:(NSMutableDictionary*) invokeParameters {
	...
	[parameters setValue:@"2009-04-15" forKey:@"Version"];
	[parameters setValue:@"1" forKey:@"SignatureVersion"];
	[parameters setValue:timeStamp forKey:@"Timestamp"];	
	...
	NSLog(@"SENT TO SERVER: %@", url);
	NSString *sContent = [NSString stringWithContentsOfURL:url];
	NSLog(@"SERVER RESPONSE: %@", sContent);
	NSData   *dContent = [sContent dataUsingEncoding:NSUTF8StringEncoding] ;
						  
	NSXMLParser *parser = [[NSXMLParser alloc] initWithData:dContent];
...
- (NSString*) getTimestamp {
	...
	[mydateformat setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
	[mydateformat setDateFormat:@"YYYY-MM-dd'T'HH:mm:ss.'000Z'"];
	NSString *dateString = [mydateformat stringFromDate:mydate];
